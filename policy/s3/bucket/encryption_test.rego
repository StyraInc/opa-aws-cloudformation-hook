...
test_deny_if_bucket_encryption_is_not_aws_kms {
	inp := create_with_properties("AWS::S3::Bucket", "MyS3Bucket", {"BucketEncryption": {"ServerSideEncryptionConfiguration": [{"ServerSideEncryptionByDefault": {"SSEAlgorithm": "aws:invalid"}}]}})

	deny["encryption not set to aws:kms for bucket: MyS3Bucket"] with input as inp
}

test_allow_if_bucket_encryption_is_set {
	inp := create_with_properties("AWS::S3::Bucket", "MyS3Bucket", {"BucketEncryption": {"ServerSideEncryptionConfiguration": [{"ServerSideEncryptionByDefault": {"SSEAlgorithm": "aws:kms"}}]}})
