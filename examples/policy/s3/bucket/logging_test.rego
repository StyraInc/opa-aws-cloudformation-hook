...
test_deny_if_logging_prefix_invalid {
	inp := create_with_properties("AWS::S3::Bucket", "MyS3Bucket", {"LoggingConfiguration": {"LogFilePrefix": "invalid"}})

	deny["logging prefix is not set correctly for bucket: MyS3Bucket"] with input as inp
}

test_deny_if_logging_bucket_invalid {
	inp := create_with_properties("AWS::S3::Bucket", "MyS3Bucket", {"LoggingConfiguration": {"DestinationBucketName": "invalid"}})
